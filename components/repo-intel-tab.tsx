...
> = {
  "actively-maintained": {
    ...
    softClassName: "bg-green-500/10 text-green-700 dark:text-green-300",
  },
  "lightly-maintained": {
    ...
    softClassName: "bg-yellow-500/10 text-yellow-700 dark:text-yellow-300",
  },
  stale: {
    label: "Stale",
    short: "Stale",
    ...
    softClassName: "bg-orange-500/10 text-orange-700 dark:text-orange-300",
  },
  abandoned: {
    label: "Abandoned",
    short: "Dead",
    ...
    softClassName: "bg-red-500/10 text-red-700 dark:text-red-300",
  },
}
...
> = {
  positive: {
    ...
    softClassName: "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300",
  },
  mixed: {
    label: "Mixed",
    short: "Mixed",
    ...
    softClassName: "bg-zinc-500/10 text-zinc-700 dark:text-zinc-300",
  },
  frustrated: {
    label: "Frustrated",
    short: "Frustrated",
...
> = {
  "production-ready": {
    ...
    softClassName: "bg-blue-500/10 text-blue-700 dark:text-blue-300",
  },
  maturing: {
    label: "Maturing",
    short: "Maturing",
    ...
    softClassName: "bg-violet-500/10 text-violet-700 dark:text-violet-300",
  },
  experimental: {
    label: "Experimental",
    short: "Experimental",
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex min-w-0 flex-col gap-1">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          AI synthesis of maintenance, community, and release signals.
        </p>
      </div>
    </div>
  )
...
function SignalBar({
  ...
  return (
    <div className="rounded-lg border border-border/70 bg-background/60 px-3 py-3 shadow-sm shadow-black/5">
      <p className="mb-1.5 text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground leading-none">
        {label}
      </p>
      <p className="mb-2 text-base font-semibold font-mono tabular-nums leading-none text-foreground">
        {value}
      </p>
      {fill !== undefined && (
...
export function RepoIntelTab({ owner, name }: RepoIntelTabProps) {
  ...
  const [intelLimit, setIntelLimit] = useState<{
  ...
  const intelTooltip =
    isIntelLimited && intelLimit.nextAllowedAt
      ...
      <div className="flex flex-col gap-4 py-2 animate-pulse">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <p className="text-xs text-muted-foreground/50">
            Fetching issues, PRs, and running AI synthesis…
          </p>
        </div>
      </div>
    )
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-xl border border-border/80 bg-background shadow-sm shadow-black/5">
        <div className="flex flex-wrap items-center justify-between gap-4 p-4">
          ...
          <div
            className={cn(
              "inline-flex w-fit items-center gap-2 rounded-md px-3 py-2 text-xs font-semibold",
              adoptionCfg.softClassName,
            )}
          >
      ...
      <div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-2">
          ...
          {metrics.commits90d !== undefined && (
            <SignalBar label="Commits (90d)" value={String(metrics.commits90d)} />
          )}
          <SignalBar label="Last Commit" value={days(metrics.daysSinceLastCommit)} />
          ...
          {metrics.avgPrMergeDays !== null && (
            <SignalBar label="Avg PR Merge Time" value={`${Math.round(metrics.avgPrMergeDays)}d`} />
          )}
          {metrics.staleIssueCount > 0 && (
            <SignalBar label="Stale Issues" value={String(metrics.staleIssueCount)} />
          )}
          {metrics.daysSinceLastRelease !== null && (
            <SignalBar label="Last Release" value={days(metrics.daysSinceLastRelease)} />
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <HealthPill
            ...
            icon={BookOpen}
            present={metrics.hasCommunityFiles.contributingGuide}
          />
          <HealthPill
            label="Code of Conduct"
            icon={Shield}
            present={metrics.hasCommunityFiles.codeOfConduct}
          />
        </div>
      </div>
        ...
        <div>
          ...
          <ul className="flex flex-col gap-2">
            {intel.topPainPoints.map((point, i) => (
              <li
                key={i}
                className="flex items-start gap-2.5 rounded-lg border border-orange-500/20 bg-orange-500/5 px-3 py-2.5"
              >
      ...
      <div className="flex gap-3 rounded-xl border border-blue-500/25 bg-blue-500/10 px-4 py-3 dark:bg-blue-500/8">
        ...
        />
        <div>
          <p className="mb-1 text-[10px] font-medium uppercase tracking-[0.18em] text-blue-700/80 dark:text-blue-400/80">
            Recommendation
          </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            variant="ghost"
            size="sm"
            className="inline-flex min-h-9 items-center gap-1.5 rounded-md px-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
          >

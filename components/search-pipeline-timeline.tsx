...
function getStepEvent(
  ...
  id: SearchPipelineStepId,
): SearchPipelineStepEvent | undefined {
...
function getStatusIndicator(status: SearchPipelineStatus, stepNumber: number) {
  ...
  return (
    <>
      {status === "running" && (
        <Loader2 className="absolute inset-0 h-full w-full animate-spin p-1 text-primary/70" />
      )}
      <span className="relative text-[11px] font-semibold tabular-nums leading-none">
        {stepNumber}
      </span>
    </>
  )
...
export function SearchPipelineTimeline({
  events,
  isSearching,
  className,
}: SearchPipelineTimelineProps) {
  if (events.length === 0 && !isSearching) return null

  const resultEvent = events.find((event) => event.type === "result")
  const errorEvent = events.find((event) => event.type === "error")
  const completedCount = PIPELINE_STEPS.filter(
    (step) => getStepEvent(events, step.id)?.status === "completed",
  ).length
  const activeStep = PIPELINE_STEPS.find(
    (step) => getStepEvent(events, step.id)?.status === "running",
  )
  const progressValue = resultEvent
    ? 100
    : Math.max(8, Math.round((completedCount / PIPELINE_STEPS.length) * 100))
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              {errorEvent?.error ??
                (resultEvent
                  ? `${resultEvent.repos.length} repos ranked in ${formatElapsed(resultEvent.elapsedMs)}`
                  : "Streaming backend progress as each stage completes.")}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-2 lg:grid-cols-3">
        {PIPELINE_STEPS.map((step, index) => {
          ...
          return (
            <div
              key={step.id}
              className="relative flex items-center gap-3 rounded-md border border-border/60 bg-background/50 p-3"
            >
              <div
                className={cn(
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {elapsed && (
                    <span className="shrink-0 text-[10px] text-muted-foreground">{elapsed}</span>
                  )}
                </div>

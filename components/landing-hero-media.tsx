...
export function LandingHeroMedia() {
  ...
  const lineClass = resolvedTheme === "light" ? "bg-black/18" : "bg-white/18"
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute right-[9%] bottom-[-6%] hidden h-[16%] w-px lg:block ${lineClass}`}
      />
      <div
        className={`absolute left-[6%] top-[16%] hidden h-4 w-4 rounded-full border lg:block ${artifactClass}`}
      />
      <div className={`absolute left-[8%] top-[18%] hidden h-px w-12 lg:block ${lineClass}`} />
      <div
        className={`absolute right-[10%] top-[24%] hidden h-3 w-14 rounded-full border lg:block ${artifactClass}`}
      />
      <div className={`absolute right-[10%] top-[24%] hidden h-8 w-px lg:block ${lineClass}`} />
      <div
        className={`absolute left-[15%] bottom-[10%] hidden h-3 w-3 rounded-full border lg:block ${artifactClass}`}
      />
      <div className={`absolute left-[15%] bottom-[10%] hidden h-px w-16 lg:block ${lineClass}`} />
      <div
        className={`absolute right-[18%] bottom-[14%] hidden h-5 w-5 rounded-sm border lg:block ${artifactClass}`}
      />

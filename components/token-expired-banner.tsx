...
export function TokenExpiredBanner({ onReconnect, message }: TokenExpiredBannerProps) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 mb-4 text-sm">
      ...
      <span className="flex-1 text-amber-700 dark:text-amber-400">
        {message ?? 'Your GitHub session has expired. Showing cached data — live sync is unavailable.'}
      </span>

  ...
  scores: z.array(
    z.object({
      ...
      relevanceScore: z
        ...
        .min(0)
        .max(10)
        .describe("How well this repo matches the original intent (0-10)"),
    }),
  ),
})
...
async function getCachedDiscoverSearch({
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(
        "id, query, normalized_query, context_hash, results, pipeline_events, result_count, cached_at, last_run_at, last_opened_at, expires_at, is_saved",
      )
      .eq("user_id", userId)
    ...
    if (!row.is_saved) {
      ...
      if (contextHash !== null && row.context_hash !== null && row.context_hash !== contextHash)
        return null
    }
  ...
  try {
    ...
    if (!existing) {
      ...
      if ((count ?? 0) >= MAX_CACHED_SEARCHES_PER_USER) {
        ...
        console.info(
          `[search-cache] skipping save for user ${userId}: row cap (${MAX_CACHED_SEARCHES_PER_USER}) reached`,
        )
        return null
      }
    }
...
function repoToSearchRepo(
  item: GitHubSearchItem,
  rank?: { evidence: string[]; relevanceScore: number },
): SearchRepo {
  return {
    id: item.id,
...
async function runSearchPipeline({
  ...
  emit?.({
    ...
    detail: token
      ? "Authenticated GitHub search is available."
      : "Using public GitHub search because the GitHub token cookie is unavailable.",
...
Include ALL repos.`,
  ...
  const shortlisted = merged
    .map((candidate) => ({
      ...candidate,
      coarseScore: coarseScoreMap.get(candidate.item.full_name) ?? 0,
    }))
...
function streamSearchPipeline({
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const cached = await getCachedDiscoverSearch({
          supabase,
          userId: user.id,
          normalizedQuery,
          contextHash,

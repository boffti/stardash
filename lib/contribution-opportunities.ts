...
function inferTypes(issue: GitHubIssue, repo: StarredRepo): ContributionType[] {
  ...
  if (includesAny(labels, DOC_LABELS) || /\b(readme|docs?|documentation|guide)\b/.test(text))
    types.add("docs")
  if (includesAny(labels, TEST_LABELS) || /\b(test|spec|coverage)\b/.test(text)) types.add("tests")
  if (includesAny(labels, BUG_LABELS) || /\bbug|fix|error|crash|broken|regression\b/.test(text))
    types.add("bugfix")
  if (includesAny(labels, INFRA_LABELS) || /\b(ci|workflow|docker|build|deploy|lint)\b/.test(text))
    types.add("infra")
  if (includesAny(labels, FEATURE_LABELS) || /\b(add|support|feature|enhancement)\b/.test(text))
    types.add("feature")
...
function inferDifficulty(issue: GitHubIssue): ContributionDifficulty {
  ...
  if (
    includesAny(labels, STARTER_LABELS) ||
    includesAny(labels, DOC_LABELS) ||
    includesAny(labels, TEST_LABELS)
  ) {
    return "beginner"
  }
...
function scoreIssue(
  ...
  const preferredTypeMatch = preferences.contributionTypes?.some((type) =>
...
export function rankReposForIssueDiscovery(
  repos: StarredRepo[],
  preferences: ContributionPreferences = {},
) {
  return [...repos]
